...
int main(void)
{
    ...
    **************************************************/
    ...
    proc_init_isr( &proc[0], main_with_return,   SVH0, RSH0, 0, &proc_stack[0][PROC_STACK_SIZE-1], 1,      1, 0 BGRT_SCHED_ARG_END );
    proc_init_isr( &proc[1], main_lb,            SVH1, RSH1, 0, &proc_stack[1][PROC_STACK_SIZE-1], LOWEST, 1, 0 BGRT_SCHED_ARG_END );
    proc_init_isr( &proc[2], main_no_return,     SVH2, RSH2, 0, &proc_stack[2][PROC_STACK_SIZE-1], 2,      2, 1 BGRT_SCHED_ARG_END );
    proc_init_isr( &proc[3], main_no_return,     SVH3, RSH3, 0, &proc_stack[3][PROC_STACK_SIZE-1], 3,      3, 0 BGRT_SCHED_ARG_END );
    proc_init_isr( &proc[4], main_no_return,     SVH4, RSH4, 0, &proc_stack[4][PROC_STACK_SIZE-1], 3,      4, 0 BGRT_SCHED_ARG_END );
    proc_init_isr( &proc[5], main_no_return,     SVH5, RSH5, 0, &proc_stack[5][PROC_STACK_SIZE-1], 3,      5, 0 BGRT_SCHED_ARG_END );
