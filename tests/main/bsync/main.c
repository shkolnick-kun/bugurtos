...
{
    status[1] = sync_wake( &sync_1, (proc_t *)0, 0 );
    proc_self_stop();
...
{
    status[2] = sync_wait( &sync_2, &proc_buff, 1 );  //Must fail on ownership.
    proc_self_stop();

    status[2] = sync_wait( &sync_2, &proc_buff, 1 );  //Must wait
    proc_self_stop();

    status[2] = sync_sleep( &sync_2 ); //Must fail.
    proc_self_stop();

    status[2] = sync_sleep( (sync_t *)0 ); //Must fail.
    proc_self_stop();

    status[2] = sync_wake( &sync_2, proc_buff, 0 );   //Must wake proc[3].
    proc_self_stop();

    status[2] = sync_wake( &sync_2, &proc[4], 0 );   //Must fail on wrong process.
    proc_self_stop();

    status[2] = sync_sleep( &sync_1 ); //Must succeed.
    proc_self_stop();
    ...
    proc_buff  = (proc_t *)0;
    status[2] = sync_wait( &sync_2, &proc_buff, 1 );
}

void main_3( void * arg )
{
    status[3] = sync_sleep( &sync_2 );
    proc_self_stop();
    ...
    status[3] = sync_wake( &sync_2, (proc_t *)0, 1 );//Chown to proc[5];
    proc_self_stop();
}

void main_4( void * arg )
{
    status[4] = sync_sleep( &sync_3 );
    proc_self_stop();
}

void main_5( void * arg )
{
    status[5] = sync_sleep( &sync_2 );
    proc_self_stop();

    status[5] = sync_wake( &sync_2, (proc_t *)0, 1 );//Must fail;
    proc_self_stop();
}
