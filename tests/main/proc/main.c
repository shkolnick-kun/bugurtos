...
int main(void)
{
    ...
    **************************************************/
    ...
    proc_init_isr( &proc[2], main_with_return,   SVH2, RSH2, 0, &proc_stack[2][BGRT_PROC_STACK_SIZE-1], 0,      2, 0 ARG_END );
    proc_init_isr( &proc[3], main_with_return,   SVH3, RSH3, 0, &proc_stack[3][BGRT_PROC_STACK_SIZE-1], 1,      2, 0 ARG_END );
    proc_init_isr( &proc[4], main_fs,            SVH4, RSH4, 0, &proc_stack[4][BGRT_PROC_STACK_SIZE-1], 2,      2, 0 ARG_END );
    proc_init_isr( &proc[5], main_wd_ss,         SVH5, RSH5, 0, &proc_stack[5][BGRT_PROC_STACK_SIZE-1], 3,      2, 1 ARG_END );
