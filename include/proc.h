...
struct _proc_t
{
    ...
    void * arg;         /*!< Аргумент для pmain, sv_hook, rs_hook, может хранить ссылку на локальные данные конкретного экземпляра процессса. */
...
Процесс завершился - произощел возврат из pmain.
*/
...
void proc_init_isr(
    ...
    code_t pmain,  /*!<Указатель на главную функцию процеса.*/
    ...
    bool_t is_rt /*!Флаг реального времени, если true, значит процесс будет иметть поведение RT.*/
#ifdef CONFIG_MP
...
/*!
\brief Запуск процеса из критической секции, либо обработчика прерывания.
...
/*!
\brief Останов процеса из критической секции или обработчика прерывания.
...
Если функцию вызывает процесс реального времени, то функци ясбрасывает его таймер.
...
#ifdef CONFIG_MP
/*!
\brief Вставка процесса в список готовых к выполнению, для внутреннего пользования.
*/
...
#endif
/*!
\brief "Низкоуровненвый" запуск процесса, для внутреннего пользования.
*/
void _proc_run( proc_t * proc );

#ifdef CONFIG_MP
/*!
\brief Вырезка процесса из списка готовых к выполнению с обновлением статистики, для внутреннего пользования.
*/
void _proc_stop_(proc_t * proc);
#else
/*!
\brief Вырезка процесса из списка готовых к выполнению, для внутреннего пользования.
*/
#define _proc_stop_(proc) gitem_cut((gitem_t *)proc)
#endif
/*!
\brief "Низкоуровненвый" останов процесса, для внутреннего пользования.
*/
void _proc_stop(proc_t * proc);

/*!
\brief Останов процесса по флагу PROC_FLG_PRE_END из критической секции или обработчика прерывания, для внутреннего пользования.
*/
void _proc_flag_stop_isr( flag_t mask );
/*!
\brief Останов процесса по флагу PROC_FLG_PRE_END, для внутреннего пользования.
*/
void _proc_flag_stop( flag_t mask );
// Упраление счетчиком захваченных ресурсов, для внутреннего пользования
/*!
\brief Инкремент счетчика захваченных ресурсов, для внутреннего пользования.
*/
void _proc_lres_inc(
...
/*!
\brief Декремент счетчика захваченных ресурсов, для внутреннего пользования.
*/
void _proc_lres_dec(
    proc_t * proc /*!< Указатель на процесс, захвативший ресурс. */
#ifdef CONFIG_USE_HIGHEST_LOCKER
...
************************************/
/*!
\brief Ленивая балансировка нагрузки, для внутренненго пользования.
...
/*!
\brief Ленивая балансировка нагрузки, локальный баланисировщик.
...
/*!
\brief Ленивая балансировка нагрузки, глобальный баланисировщик.
